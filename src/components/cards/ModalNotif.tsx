...
import { useNotif } from "../layouts/NotifProvider"
const NotifModal = ({
    open,
    handlerOpen
}: {
    ...
    const handlerRead = async (req: string) => {
        try {
            setLoading("error")
    ...
    const handlerRemove = async (req: string) => {
        try {
            setLoading("loading")
            ...
            <div>
                <List
                    ...
                    renderItem={(item: INotification, index) => (
                        <List.Item
                            actions={[
                                <button disabled={isLoading === "loading"} onClick={() => handlerRead(item.id)}>Tandai</button>,
                                <button disabled={isLoading === "loading"} onClick={() => handlerRemove(item.id)}>Hapus</button>
                            ]}
                        >
                            <List.Item.Meta
                                avatar={
