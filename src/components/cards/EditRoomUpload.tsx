...
const EditRoomUpload: React.FC<RoomUploadProps> = ({ drawerHandler, drawer }) => {
    ...
    const handlerInputFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
        try {
            if (!e.target.files)
                return
    ...
    const handlerDelete = async (imageId: string, index: number) => {
            ...
            <div className='flex flex-wrap w-full gap-3 '>
                {
                    fileList.map((img: any, index: number) => (
                        <div className='bg-slate-400 h-[100px] w-[100px] rounded-md relative flex flex-direction'>
                            ...
                            {
                                fileList.length > 1 &&
                                <button disabled={index === isLoading.index} onClick={() => handlerDelete(img.id, index)} className='absolute py-1 top-2 left-2 px-2 text-[12px] hover:bg-red-500/50 rounded-full bg-slate-400/50 text-white'><DeleteFilled /></button>
                            }
                        </div>
                    ))
                }
                {
                    fileList.length >= 6 ?
                        ...
                        <div className='w-[100px] h-[100px] border-2 rounded-md border-dashed border-slate-300 bg-slate-100 cursor-pointer flex justify-center hover:border-sky-400 items-center'>
                            <input accept=".png, .jpg, .jpeg" className='w-[100px] h-[100px] cursor-pointer relative opacity-0 after:bg-slate-600 after:w-[100px] after:h-[100px] ' type="file" onChange={(e) => handlerInputFile(e)} />
                            <span className='absolute text-2xl text-slate-400 pointer-events-none'><FileAddOutlined /></span>
                        </div>
                }
            </div>
        </div>
