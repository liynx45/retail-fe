import { Pagination, Popconfirm, Table, TableColumnsType, message } from 'antd';
...
const Facility = () => {
  ...
  const handleOpen = (id?: string) => {
    const existing = facility.data.find((facility: IFacility) => facility.id === id)
    setToggleOpen(prev => ({
  ...
  const changeItem = (page: number, take: number) => {
    seacrhParams.set("page", page.toString())
    seacrhParams.set("take", take.toString())
    setSearchParams(seacrhParams)
    dispatch(fetchfacility(`?page=${page}&take=${take}`))
  }
  ...
  return (
    <div className='flex gap-4'>
      <div className='w-[75%]'>
        <Table
          columns={column}
          dataSource={facility.data.map((data: IFacility) => ({
            id: data.id,
            name: data.name,
        ...
        />
        <EditFacility key={new Date().getTime()} handleOpen={handleOpen} data={toggleOpen} />
        <Pagination
