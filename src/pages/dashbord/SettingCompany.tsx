...
import { Button, Cascader, Form, Input, InputNumber, Modal, message } from 'antd'
...
function SettingCompany() {
    ...
    const handlerAddress = async (data: any[]) => {
        switch (data.length) {
            case 1:
                const kab = await getAddress("kabupaten/", (data[0] + ".json"))
                if (kab) {
                    setFilteredOptions(filteredOptions.map((prov: any) => {
                ...
                if (kec) {
                    setFilteredOptions(filteredOptions.map((prov: any) => {
                        if (prov.value === data[0]) {
                            return {
                                ...prov,
                                children: prov.children.map((kab: any) => {
                                    if (kab.value === data[1]) {
                                        return {
                                            ...kab,
                                            children: kec
                            ...
                            return {
                                ...
                                children: prov.children.map((kab: any) => {
                                    if (kab.value === data[1]) {
                                        return {
                                            ...kab,
                                            children: kab.children.map((kec: any) => {
                                                if (kec.value === data[2]) {
                                                    return {
                                                        ...kec,
                                                        children: lurah
    ...
    const handlerCascader = (data: any, vol: any) => {
        handlerAddress(data)
            ...
            >
                ...
                <div className='w-full grid items-center gap-4 grid-cols-2'>
                    ...
                    <div>
                        <Form.Item
                        label="Banner">
                            <button onClick={() => setBanner(true)} className='px-4 py-1 w-full bg-sky-400 rounded-md'>Edit banner</button>
                        </Form.Item>
                        <Modal
                            open={banner}
                        >
                ...
                <Form.Item
                    ...
                    name="address"
                >
                    <Cascader options={filteredOptions} onChange={handlerCascader} placeholder="Please select" />
                </Form.Item>
